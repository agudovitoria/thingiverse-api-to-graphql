...
export default class LoginController {
  ...
  codeExchange(request: Request, response: Response) {
    ...
    accessTokenObtained
      ...
      .catch(({ message }: Error) =>
        response.status(StatusCodes.INTERNAL_SERVER_ERROR).send({ message })
      );
  }
  ...
  private getAuthorizeUrl(): string {
    ...
    const loginRedirectUrl: string = this.envs.getEnvByName(
      "login_redirect_url"
    );
  ...
  private extractTokenFromRequest(request: Request): string {
    ...
    if (!token) {
      throw new Error("Invalid or not found token on request");
    }
